...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            return scheme_eval(MacroProcedure.apply_macro(operator, rest, env), env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        eval_value = scheme_eval(expressions.rest.first, env)
        env.define(symbol, eval_value)
        return symbol
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        current_name = current_binding.first
        current_value = scheme_eval(current_binding.rest.first, env)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(expression):
        validate_form(expressions, 2, 2)
        env.define(expression, scheme_eval(expressions.rest.first, env))
        return expression
    if isinstance(expression, Pair) and scheme_symbolp(expression.first):
        procedure = MacroProcedure(expression.rest, expressions.rest, env)
        env.define(expression.first, procedure)
        return expression.first
    else: raise SchemeError
    # END Problem 20
