...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    a = [p for p in paragraphs if select(p)]
    if k >= len(a):
        return ''
    return a[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def p(pstring):
        pstring = remove_punctuation(pstring).lower().split()
        for elem in topic:
            for word in pstring:
                if elem == word:
                    return True
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0:
        ...
        correct = 0
        for l in range(min(len(reference_words), len(typed_words))):
            if typed_words[l] == reference_words[l]:
                correct += 1
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if len(start) == 0 or limit < 0 or len(goal) == 0:
...
def meowstake_matches(start, goal, limit):
    ...
    elif goal[0] == start[0]: # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9    
    "*** YOUR CODE HERE ***"
    return game(words, [[times[i] - times[i - 1] for i in range(1, len(times))] for times in times_per_player])
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for i in words:
        ...
        for p in players:
            if x > time(game, p, i):
                x, index = time(game, p, i), p
        lst[index].append(word_at(game, i))
    return lst
    # END PROBLEM 10
...
def faster_autocorrect(user_word, valid_words, diff_function, limit):
    ...
    key = (user_word, diff_function, limit)
    if user_word in valid_words:
        return user_word
    if key in dnary:
        return dnary[key]
    ...
    for word in valid_words:
        temp = diff_function(user_word, word, limit)
        if temp <= limit:
            diff_from_input[word] = temp
    if diff_from_input:
        dnary[key] = min(diff_from_input, key=diff_from_input.get)
    return dnary[key]
    # END PROBLEM EC2
