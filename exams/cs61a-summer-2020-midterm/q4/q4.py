...
def schedule(slideshow, sum_to, max_digit):
    ...
    def schedule_helper(slideshow, sum_sofar, index):
        if sum_sofar == sum_to and index == len(slideshow):
            return [slideshow]
        elif index == len(slideshow):
            return []
        elif slideshow[index] != "&":
            return schedule_helper(slideshow, sum_sofar + int(slideshow[index]), index + 1)
        ans = []
        for x in range(max_digit + 1):
            modified_slideshow = slideshow[:index] + str(x) + slideshow[index + 1:]
            ans.extend(schedule_helper(modified_slideshow, sum_sofar + x, index + 1))
        return ans
