...
def missing_digits(n):
    ...
    def doogie(num, next):
        if num == 0:
            ...
            return doogie(num // 10, next)
        else:
            return next - num % 10 - 1 + doogie(num // 10, num % 10)
    return doogie(n, n % 10)
...
def count_change(total):
    ...
    def poops(m, total):
        if total < 0:
            ...
            with_m = poops(m, total - m)
            without_m = poops(2 * m, total)
            return with_m + without_m
    return poops(1, total)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda x: lambda y: x(x, y))(lambda x, y: y if y == 1 else mul(y, (x, sub(y, 1))))