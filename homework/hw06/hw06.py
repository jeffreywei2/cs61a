...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        if self.year < Mint.current_year - 50:
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if bst_max(t.branches[0]) <= t.label and bst_max(t.branches[1]) >= t.label and is_bst(t.branches[0]) and is_bst(t.branches[1]):
            return True
        return False
