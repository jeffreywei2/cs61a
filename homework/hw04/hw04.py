...
def balanced(m):
    ...
    if is_planet(m):
        ...
        leftend, rightend = end(left(m)), end(right(m))
        tleft, tright = length(left(m)) * total_weight(leftend), length(right(m)) * total_weight(rightend)
        return tleft == tright and balanced(leftend) and balanced(rightend)
